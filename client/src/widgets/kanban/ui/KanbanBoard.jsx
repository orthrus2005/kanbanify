...
export const KanbanBoard = ({ showInbox = true, inboxOnly = false, showHeader = true, inboxExpanded = false, splitMiddlePanel = null }) => {
  ...
  const handleDragEnd = async (event) => {
    ...
    const targetColumnId =
      overParsed.type === 'column' ? overParsed.id : overParsed.type === 'task' ? tasks.find((item) => item.id === overParsed.id)?.column_id : null;
  ...
  return (
    ...
    >
      <section
        className={`${shouldRenderSplitPanels ? 'flex h-full min-h-0 flex-col overflow-hidden bg-transparent' : `kb-board flex h-full min-h-0 flex-col ${showInbox ? 'overflow-visible' : 'overflow-hidden'}`}`}
      >
        {showHeader ? (
        ...
        {shouldRenderSplitPanels ? (
          <div className="relative z-[1] flex flex-1 min-h-0 items-stretch gap-4 overflow-hidden px-3 pb-0 pt-1 sm:px-6 sm:pb-0 sm:pt-2">
            <InboxWorkspace />

            {splitMiddlePanel}

            <BoardWorkspace>{renderBoardColumns()}</BoardWorkspace>
          </div>
        ) : (
          <div
          ...
          >
            <div className={`items-start ${inboxOnly ? (inboxExpanded ? 'h-full w-full min-w-0' : 'w-full max-w-[760px]') : 'flex min-h-full min-w-max gap-3 sm:gap-5'}`}>
              {showInbox ? <InboxColumn expanded={inboxExpanded} /> : null}
              {shouldRenderBoardColumns ? renderBoardColumns() : null}
            </div>
          </div>
        )}
      </section>
